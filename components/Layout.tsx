...
import { Home as HomeIcon, Search, ShoppingBag, User, Sparkles, Bell, Menu, X, ChevronRight, Star, ShieldCheck, Truck, RotateCcw, FileText, MessageSquare, HelpCircle } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, cartCount }) => {
  ...
  // New Logo Source: Represents the uploaded phone case icon
  ...
  const menuItems = [
    ...
    { label: "Privacy Policy", icon: <ShieldCheck size={18} />, path: "/contact" },
    { label: "Return & Refund Policy", icon: <RotateCcw size={18} />, path: "/contact" },
    { label: "Terms & Conditions", icon: <FileText size={18} />, path: "/contact" },
    { label: "Shipping Policy", icon: <Truck size={18} />, path: "/contact" },
    { label: "Divider", type: "divider" },
    { label: "Reviews", icon: <MessageSquare size={18} />, path: "/contact" },
    { label: "Contact Us", icon: <User size={18} />, path: "/contact" },
    { label: "FAQ", icon: <HelpCircle size={18} />, path: "/contact" },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24 md:pb-0 md:pl-24">
        ...
        <div className="flex flex-col h-full p-6">
          <div className="flex items-center justify-between mb-8 px-2">
            <Link to="/" onClick={() => setIsDrawerOpen(false)} className="flex items-center">
              {/* Force logo to black for the light drawer background */}
          ...
          <div className="mt-6 p-4 liquid-glass-dark rounded-3xl text-white">
            {/* Invert logo for the dark container background */}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between liquid-glass rounded-2xl h-14 px-4 border-white/60">
          <div className="flex items-center space-x-3">
            ...
            <Link to="/" className="flex items-center">
              {/* Ensure logo is black in the header */}
          ...
          <div className="flex items-center space-x-2">
            ...
            <button className="p-2 text-slate-800 hover:bg-white/40 rounded-full transition-all md:hidden">
      ...
      <nav className="hidden md:flex fixed left-4 top-4 bottom-4 w-20 flex-col items-center py-8 space-y-10 liquid-glass rounded-[2.5rem] z-50">
        ...
        <NavIcon to="/contact" icon={<User size={24} />} active={isActive('/contact')} label="Account" />
      </nav>

      {/* Mobile Bottom Tab Bar with Liquid Glass */}
      <nav className="md:hidden fixed bottom-6 left-4 right-4 h-16 liquid-glass rounded-[2rem] flex items-center justify-between px-6 z-[60] pb-safe shadow-2xl border-white/40">
        ...
        <MobileTab to="/contact" icon={<User size={22} />} active={isActive('/contact')} />
      </nav>
    </div>
  );
};
